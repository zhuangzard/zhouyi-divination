...
description: 周易综合占卜系统 — 基于用户八字命盘，结合多种传统术数（梅花易数、马前课、六爻、奇门遁甲等）进行专业命理分析和事件占卜。每次占卜由六位专家会诊，给出理性、专业、不奉承的综合判断。
user_invocable: true
---
...
**文件内容**：包含占卜时间、问题、卦象、六专家会诊结论、建议、注意事项。格式参考步骤5的输出格式。
...
5. **六位专家会诊**：每次分析必须模拟六位不同流派/视角的专家，各自独立给出判断，最后综合
